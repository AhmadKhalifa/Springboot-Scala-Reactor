...
object DataFactory {
    ...
    def generateCredentialsDto(username: String = generateString, password: String = generateString): CredentialsDTO =
        new CredentialsDTO(username, password)

    def generateUser(username: String = generateString,
                     password: String = generateString): User = new User(username, password, UserRole.CUSTOMER)
