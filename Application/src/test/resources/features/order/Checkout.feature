Feature: Checkout
  ...
  Scenario: 2.1. Checkout with an invalid basket (one item not available)
    ...
    When customer checks-out the basket
    Then an error is returned to the user with status code 400, minor code 10, and a message "Sorry, one or more items are not available, let's try something else"
