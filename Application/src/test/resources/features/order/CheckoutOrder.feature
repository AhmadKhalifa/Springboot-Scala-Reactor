Feature: Checkout order
  ...
  Scenario: 1. Checkout with a valid basket
    Given customer has the following items in basket
      | Item name        | Amount | Price per item | Is item available |
  ...
  Scenario: 2.1. Checkout with an invalid basket (one item not available)
    ...
    When customer checks-out the basket
    Then an error is returned to the user with status code 400, minor code 10, and a message "Sorry, Pepsi can is not available, let's try something else"
  ...
  Scenario: 3. Checkout with an invalid basket (total amount is less than the minimum)
    Given customer has the following items in basket
      | Item name        | Amount | Price per item | Is item available |
      | Pepsi can        | 5      | 10             | false	       |
    When customer checks-out the basket
  ...
  Scenario: 4. Checkout with an invalid basket (total amount is more than the maximum)
    Given customer has the following items in basket
      | Item name        | Amount | Price per item | Is item available |
      | Mix cheese pizza | 10     | 150            | true	       |
      | Pepsi can        | 5      | 10             | true	       |
    When customer checks-out the basket
    Then an error is returned to the user with status code 400, minor code 10, and a message "Having a party without me? let's break the order down into smaller orders"
